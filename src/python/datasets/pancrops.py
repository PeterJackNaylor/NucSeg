...
class pancrops(BaseData):
    def generate_filename(self):
        ...
        """
        file_pattern = os.path.join(self.path, "{}_crop.png")
        file_gt_pattern = file_pattern.replace("crop.", "labeled_mask_corrected.")
        ...
        for i in range(1, 1366):
            if i not in invalid_index:
                raw_path = file_pattern.format(i)
                gt__path = file_gt_pattern.format(i)
