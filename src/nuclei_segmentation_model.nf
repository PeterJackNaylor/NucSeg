...
TRAINED_MODELS .map{ it0, it1, it2, history, it4, it5 -> [it0, it1, it2, history, it4, it5, history.splitCsv(header: ["c1","c2","c3","c4","c5","c6","c7","c8","val_score","c9","c10","c11"], skip:1).view{p -> p}.map { row -> Float.valueOf("${row.val_score}") } .max () .val]}
               .view{ p -> p }
               .set{TRAINED_MODELS}
