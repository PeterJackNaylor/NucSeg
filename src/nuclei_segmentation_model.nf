...
process validation_with_ws {
    ...
    script:
    f1_score = Channel.fromPath(history).splitCsv(header: ["c1","c2","c3","c4","c5","c6","c7","c8","val_score","c9","c10","c11"], skip:1).map { row -> Float.valueOf("${row.val_score}") } .max () .view {it -> it} .val
    """
